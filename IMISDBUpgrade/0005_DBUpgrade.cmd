...
@ECHO.
@ECHO ===================================================
@ECHO = iMIS 20.2.9.x Database Upgrade Driver Script    =
@ECHO ===================================================
@ECHO.
...
REM ---------------------------------------------------------- 
SET PREVVERSTR=20.2.9
SET PREVVERVALUE=200209

REM * SET IDENTIFIER VVVV------- **
SET UPGRADESCRIPTDIR=0005_DBUpgrade
...
if "%LISTOFOBJECTS%" NEQ "" (
    ...
    for %%i in ("%LISTOFOBJECTS:|=" "%") do (
        "%BINDIR%\BuildPublishBOs.exe" -CS %SERVERNAME% -CU %USERNAME% -CP %PASSWORD% -CD %DBNAME% -O %%~i  | .\Utilities\Tee /A /FOE:: .\LogFiles\%UPGRADESCRIPTDIR%\BuildPublishDynamicBOs.log 
    )
) else (
...
REM -----------------------------------------------------------------------
REM - Convert Public View Navigation based on removal of all Public Views -
REM -----------------------------------------------------------------------
@ECHO.
IF "%LOGTIME%" EQU "1" ECHO %date%-%time%
@ECHO Removing References To Removed Public View Pages ...
"%BINDIR%\RemovePublicViewUtility.exe" -CS %SERVERNAME% -CU %USERNAME% -CP %PASSWORD% -CD %DBNAME% | .\Utilities\Tee .\LogFiles\%UPGRADESCRIPTDIR%\RemovePublicViewUtility.log
IF %ERRORLEVEL% NEQ 0 (
    @echo.
    @echo *** ERROR removing references to removed public view pages ***
    @echo.
    @echo *** PROCESS ENDED WITH ERRORS !!! ***
    @echo.
    EXIT /B 1
...
REM --------------------------------------------------------------------
REM - Converting Multiply-Scheduled Task Triggers to Separate Triggers -
REM --------------------------------------------------------------------
@ECHO.
IF "%LOGTIME%" EQU "1" ECHO %date%-%time%
@ECHO Converting Multiply-Scheduled Task Triggers to Separate Triggers ...
"%BINDIR%\SplitTasks.exe" -CS %SERVERNAME% -CU %USERNAME% -CP %PASSWORD% -CD %DBNAME% 1> .\LogFiles\%UPGRADESCRIPTDIR%\SplitTasks.log 2> .\LogFiles\%UPGRADESCRIPTDIR%\SplitTasks.err
IF %ERRORLEVEL% NEQ 0 (
    @echo.
    @echo *** ERROR converting multiply-scheduled tasks ***
    @echo.
    @echo *** PROCESS ENDED WITH ERRORS !!! ***
    @echo.
    EXIT /B 1
