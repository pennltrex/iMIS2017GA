...
@ECHO.
@ECHO ===================================================
@ECHO = iMIS 15.2.23.x Database Upgrade Driver Script   =
@ECHO ===================================================
@ECHO.
...
REM ---------------------------------------------------------- 
SET PREVVERSTR=15.2.23
SET PREVVERVALUE=150223

REM * SET IDENTIFIER VVVV------- **
SET UPGRADESCRIPTDIR=0003_DBUpgrade
...
REM --------------------------------------------------------------------------
REM - Rebuild Gift History Summary
REM --------------------------------------------------------------------------
@ECHO.
IF "%LOGTIME%" EQU "1" ECHO %date%-%time%
@ECHO Rebuilding Gift History Summary ...
osql -t -S %SERVERNAME% -U %USERNAME% -P %PASSWORD% -d %DBNAME% -n -e -b -r -w 256 -i .\ScriptFiles\%UPGRADESCRIPTDIR%\RebuildGiftHistorySummary.sql 1> .\LogFiles\%UPGRADESCRIPTDIR%\RebuildGiftHistorySummary.log 2> .\LogFiles\%UPGRADESCRIPTDIR%\RebuildGiftHistorySummary.err
IF %ERRORLEVEL% NEQ 0 (
    @echo.
    @echo *** ERROR rebuilding gift history summary ***
    @echo.
    type ".\LogFiles\%UPGRADESCRIPTDIR%\RebuildGiftHistorySummary.err"
    @echo.
    @echo *** UPGRADE PROCESS ENDED WITH ERRORS !!! ***
    @echo.
    EXIT /B 1
