...
WITH    c AS ( SELECT   a.[CustomerId],
                        ...
                        CASE WHEN a.[PaymentTermsKey] IS NULL THEN COALESCE(i.[DUE_DATE], StartDate)
                             ELSE
                                CASE WHEN pt.[PaymentIntervalTypeCode] = 1 THEN DATEADD(MM,-9, i.[INSTALL_BILL_DATE]) 
               ...
               FROM     [AutoPayInstruction] a
                        ...
                        WHERE a.[Status] = 'Active' AND a.[SourceSystem] = 'FR'
             )
    SELECT  CustomerId ,
            InstructionId ,
            Distribution ,
...
AS
  SELECT
    ...
    WHERE a.InstructionId =
    (
        SELECT TOP 1 a2.InstructionId
...
AS
     SELECT DISTINCT d.InvoiceNumber
      ...
      WHERE a.[Status] IN ('A','I','R') OR a.[Attempt] > 2
...
AS
       SELECT i.[Balance]
              ...
              [Name].[Company]
         FROM [Invoice] i
              INNER JOIN [vCurrentAutoPayDuesInstruction] api ON i.[ST_ID] = api.CustomerId
...
SELECT i.[Balance] - COALESCE(ppa.[PendingAmount],0) AS [PaymentAmount],
   ...
   FROM [dbo].[Invoice] i
        INNER JOIN [dbo].[AutoPayInstruction] api ON i.[CUSTOMER_REFERENCE] = CAST(api.[InstructionId] AS varchar(40))
        ...
        LEFT OUTER JOIN (SELECT vppa.[INVOICE_REFERENCE_NUM], 
                                SUM(vppa.[Amount]) AS PendingAmount 
...
AS
SELECT vAutoPayInvoicesFundraising.code_Token AS Code_Token,vAutoPayInvoicesFundraising.Company,vAutoPayInvoicesFundraising.Description,vAutoPayInvoicesFundraising.DonorID,vAutoPayInvoicesFundraising.DUE_DATE,vAutoPayInvoicesFundraising.FullName,vAutoPayInvoicesFundraising.InvoiceNumber,vAutoPayInvoicesFundraising.PaymentAmount,vAutoPayInvoicesFundraising.InstructionId,vAutoPayInvoicesFundraising.DataVaultGatewayAccountKey,vAutoPayInvoicesFundraising.ReconcileFlag
...
SET NUMERIC_ROUNDABORT OFF
GO
CREATE VIEW [vCertificationProgramRegistrationWithStatus]
AS
SELECT cpr.[CertificationProgramRegistrationKey],
       ...
       END AS [Status],
       CASE WHEN cpr.[RegistrationStatusCode] = 'AP' THEN LastUpdate.[StatusChangedOn]
       END AS [EarnedDate]
...
-- new vSoaGroupMemberSummaryProduct
...
SELECT [GroupMemberDetailId],
  ...
  FROM [dbo].[vSoaGroupMemberDetailProduct] WITH (NOEXPAND);
...
SELECT m.[MEETING] AS [EventId],
       ...
       m.[MEETING] AS [CategoryId],
       m.[MEETING] AS [CategoryName],
       m.[STATUS] AS [Status],
       ...
       CAST(CASE 
              WHEN m.[WEB_VIEW_ONLY] = 1 THEN 1
              WHEN m.[EMAIL_REGISTRATION] = 1 THEN 1
              WHEN (SELECT COUNT(1) FROM Product p WHERE p.PRODUCT_MAJOR = m.MEETING) > 0 THEN 3
              ELSE 1
            END
          AS INT) 
  ...
  FROM [dbo].[Meet_Master] as m LEFT OUTER JOIN [dbo].[EventDetails] as d ON m.MEETING = d.MEETING
...
SELECT n.[ID] AS [PartyId],
       ...
       (SELECT TOP 1 c.[ContactKey] FROM [dbo].[ContactMain] c WHERE n.[ID] = c.[ID]) AS [UniformId]
  FROM [dbo].[Name] n
