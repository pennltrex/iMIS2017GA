...
BEGIN
    ...
    BEGIN
        ...
        IF (@publishedOn IS NOT NULL)    
            EXEC [dbo].[asi_AddToPublishQueue] @publishedOn, @publishedByUserKey, 3;
    END
    IF EXISTS(SELECT 1 FROM deleted WHERE DocumentTypeCode = N'CON') AND NOT EXISTS(SELECT 1 FROM inserted WHERE DocumentTypeCode = N'CON')
    BEGIN
...
BEGIN
    ...
    -- Mark any associatd 'User' record as disabled, as long as it's not a 'special' user
    UPDATE um
       SET [IsDisabled] = 1,
           [ContactMaster] = '',
           [UserId] = ''
