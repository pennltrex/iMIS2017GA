...
IF EXISTS (SELECT 1 FROM sys.triggers WHERE object_id = OBJECT_ID(N'[dbo].[asi_ContactMain_Insert_Update_Delete]'))
...
CREATE TRIGGER [dbo].[asi_ContactMain_Insert_Update_Delete] 
    ON [dbo].[ContactMain]
...
BEGIN
    ...
    BEGIN
        SELECT @publishedOn = MIN(UpdatedOn) FROM deleted;
        SELECT TOP 1 @publishedByUserKey = [UpdatedByUserKey] FROM deleted WHERE [UpdatedOn] = @publishedOn
    END
    ELSE
        SELECT TOP 1 @publishedByUserKey = [UpdatedByUserKey] FROM inserted WHERE [UpdatedOn] = @publishedOn
    IF (@publishedOn IS NOT NULL)    
        EXEC [dbo].[asi_AddToPublishQueue] @publishedOn, @publishedByUserKey, 5;
END;
...
IF EXISTS (SELECT 1 FROM sys.triggers WHERE object_id = OBJECT_ID(N'[dbo].[asi_DocumentMain_Insert_Update_Delete]'))
...
CREATE TRIGGER [dbo].[asi_DocumentMain_Insert_Update_Delete] 
    ON [dbo].[DocumentMain]
    FOR INSERT, UPDATE, DELETE
AS
BEGIN
    ...
    BEGIN
        ...
        IF (@publishedOn IS NOT NULL)    
            EXEC [dbo].[asi_AddToPublishQueue] @publishedOn, @publishedByUserKey, 3;
    END
    ELSE IF EXISTS(SELECT 1 FROM deleted WHERE DocumentTypeCode = N'POS')
    BEGIN
        ...
        IF (@publishedOn IS NOT NULL)    
            EXEC [dbo].[asi_AddToPublishQueue] @publishedOn, @publishedByUserKey, 3;
    END
    IF EXISTS(SELECT 1 FROM inserted WHERE DocumentTypeCode = N'CON')
    BEGIN
        SELECT @publishedOn = MIN(UpdatedOn) FROM inserted WHERE DocumentTypeCode = N'CON';
        SELECT TOP 1 @publishedByUserKey = [UpdatedByUserKey] FROM inserted WHERE DocumentTypeCode = N'CON' AND [UpdatedOn] = @publishedOn
        IF (@publishedOn IS NOT NULL)    
            EXEC [dbo].[asi_AddToPublishQueue] @publishedOn, @publishedByUserKey, 4;
    END
    ELSE IF EXISTS(SELECT 1 FROM deleted WHERE DocumentTypeCode = N'CON')
    BEGIN
        SELECT @publishedOn = MIN(UpdatedOn) FROM deleted WHERE DocumentTypeCode = N'CON';
        SELECT TOP 1 @publishedByUserKey = [UpdatedByUserKey] FROM deleted WHERE DocumentTypeCode = N'CON' AND [UpdatedOn] = @publishedOn
        IF (@publishedOn IS NOT NULL)    
            EXEC [dbo].[asi_AddToPublishQueue] @publishedOn, @publishedByUserKey, 4;
    END
    ...
    BEGIN
        ...
        IF (@publishedOn IS NOT NULL)    
            EXEC [dbo].[asi_AddToPublishQueue] @publishedOn, @publishedByUserKey, 6;
    END
    ELSE IF EXISTS(SELECT 1 FROM deleted WHERE DocumentTypeCode = N'CTY')
    BEGIN
...
BEGIN
    DECLARE @publishedOn datetime;
    SET @publishedOn = GETDATE();

    -- Add a row to PublishQueue for every Product being inserted as new
    INSERT INTO [dbo].[PublishQueue]([PublishedOn], [Status], [Type], [ItemCode], [ItemAction])
        SELECT @publishedOn, 0, 0, i.MEETING, 0
          FROM inserted i
               LEFT OUTER JOIN deleted d ON i.MEETING = d.MEETING
         WHERE d.MEETING IS NULL  
    -- Add a row to PublishQueue for every Product being updated
    INSERT INTO [dbo].[PublishQueue]([PublishedOn], [Status], [Type], [ItemCode], [ItemAction])
        SELECT @publishedOn, 0, 0, i.MEETING, 1
          FROM inserted i
               INNER JOIN deleted d ON i.MEETING = d.MEETING
    -- Add a row to PublishQueue for every Product being deleted
    INSERT INTO [dbo].[PublishQueue]([PublishedOn], [Status], [Type], [ItemCode], [ItemAction])
        SELECT @publishedOn, 0, 0, d.MEETING, 2
          FROM deleted d
...
BEGIN 
    SET NOCOUNT ON
    INSERT INTO [dbo].[PciAuditLog] ([UserId],[UserKey],[EventType],[Result],[OriginationCode],[CC_LAST4],[ENCRYPT_CC_NUMBER])
        SELECT [UserId],[UserKey],[EventType],[Result],[OriginationCode],[CC_LAST4],[ENCRYPT_CC_NUMBER]
        FROM inserted i 
        WHERE NOT EXISTS(SELECT 1 
                           FROM [dbo].[PciAuditLog] t 
                          WHERE i.[SqlLogin] = t.[SqlLogin]
...
-- Create a Temp Table with all values to be passed to the StoredProcedure
       CREATE TABLE #TransTemp(TransNum int, InvoiceNum int, DonorId varchar(10), SourceSystem varchar(10), TransSourceSystem varchar(10))

        -- Populate Temp Table with the values to be passed to the Stored Procedure
        INSERT INTO #TransTemp (TransNum, InvoiceNum, DonorId, SourceSystem, TransSourceSystem)
    ...
    BEGIN
        ...
        IF EXISTS (SELECT 1 FROM #TransTemp WHERE SourceSystem = 'FR' OR ((SourceSystem IS NULL OR SourceSystem = '') AND TransSourceSystem = 'FR'))
        BEGIN
            EXEC dbo.asi_UpdateFundraisingViewTables_FR 
        END
        -- 2013-08-29 - (SBT 39818) Added "(...OR SourceSystem = '')" to account for new empty condition
        IF EXISTS (SELECT 1 FROM #TransTemp WHERE SourceSystem IN ('DUES','SC') OR ((SourceSystem IS NULL OR SourceSystem = '') AND TransSourceSystem IN ('DUES','SC')))
        BEGIN
            EXEC dbo.asi_UpdateFundraisingViewTables_DUES 
        END
        -- 2013-08-29 - (SBT 39717) Changed 'MEET' to 'MEETING'
        IF EXISTS (SELECT 1 FROM #TransTemp WHERE SourceSystem = 'MEETING')
        BEGIN
...
-- Create a Temp Table with all values to be passed to the StoredProcedure
    ...
    IF @countDonors = 1
    BEGIN
        IF EXISTS (SELECT 1 FROM #TransTemp WHERE SourceSystem = 'FR' OR (SourceSystem IS NULL AND TransSourceSystem = 'FR'))
        BEGIN
            EXEC dbo.asi_UpdateFundraisingViewTables_FR 
        END
        IF EXISTS (SELECT 1 FROM #TransTemp WHERE SourceSystem IN ('DUES','SC') OR (SourceSystem IS NULL AND TransSourceSystem IN ('DUES','SC')))
        BEGIN
            EXEC dbo.asi_UpdateFundraisingViewTables_DUES 
        END
        -- 2013-08-29 - (SBT 39717) Changed 'MEET' to 'MEETING'
...
BEGIN
    ...
    IF EXISTS (
        SELECT 1 
          FROM inserted i INNER JOIN deleted d ON i.TRANS_NUMBER = d.TRANS_NUMBER AND i.LINE_NUMBER = d.LINE_NUMBER AND i.SUB_LINE_NUMBER = d.SUB_LINE_NUMBER
         WHERE i.AMOUNT <> d.AMOUNT
            ...
            OR i.ACTIVITY_SEQN <> d.ACTIVITY_SEQN
...
BEGIN
    ...
    -- Insert CommunityInvolvement
    IF UPDATE([CommunityInvolvement])
        INSERT INTO [dbo].[UserDefinedSingleInstanceProperty] ([TableName], [PropertyName], [RowID], [RowKey], [PropertyStringValue])
            SELECT 'Profile_Page', 'CommunityInvolvement', [ContactKey], '00000000-0000-0000-0000-000000000000', [CommunityInvolvement]
              FROM inserted

    -- Insert Awards
    IF UPDATE([Awards])
        INSERT INTO [dbo].[UserDefinedSingleInstanceProperty] ([TableName], [PropertyName], [RowID], [RowKey], [PropertyStringValue])
            SELECT 'Profile_Page', 'Awards', [ContactKey], '00000000-0000-0000-0000-000000000000', [Awards]
              FROM inserted
...
BEGIN
    ...
    BEGIN
        -- Update row if it exists and setting to non-null
        UPDATE t
           SET t.[PropertyStringValue] = i.[CommunityInvolvement]
          FROM [dbo].[UserDefinedSingleInstanceProperty] t
               INNER JOIN inserted i ON t.[RowID] = i.[ContactKey]
         WHERE t.[TableName] = 'Profile_Page' AND t.[PropertyName] = 'CommunityInvolvement' AND i.[CommunityInvolvement] IS NOT NULL
        -- Insert row where property doesn't exist and setting to non-null
        INSERT INTO [dbo].[UserDefinedSingleInstanceProperty] ([TableName], [PropertyName], [RowID], [RowKey], [PropertyStringValue])
            SELECT 'Profile_Page', 'CommunityInvolvement', i.[ContactKey], '00000000-0000-0000-0000-000000000000', i.[CommunityInvolvement]
              FROM inserted i INNER JOIN deleted d ON i.[ContactKey] = d.[ContactKey]
             WHERE i.[CommunityInvolvement] IS NOT NULL AND d.[CommunityInvolvement] IS NULL
                   AND NOT EXISTS (SELECT 1 FROM [dbo].[UserDefinedSingleInstanceProperty] s
                                    WHERE s.[TableName] = 'Profile_Page' AND s.[PropertyName] = 'CommunityInvolvement' AND s.[RowID] = i.[ContactKey])
        -- Delete row if setting to null
        DELETE t
          FROM [dbo].[UserDefinedSingleInstanceProperty] t
               INNER JOIN inserted i ON t.[RowID] = i.[ContactKey]
         WHERE t.[TableName] = 'Profile_Page' AND t.[PropertyName] = 'CommunityInvolvement' AND i.[CommunityInvolvement] IS NULL
    END
    ...
    BEGIN
        -- Update row if it exists and setting to non-null
        UPDATE t
           SET t.[PropertyStringValue] = i.[Awards]
          FROM [dbo].[UserDefinedSingleInstanceProperty] t
               INNER JOIN inserted i ON t.[RowID] = i.[ContactKey]
         WHERE t.[TableName] = 'Profile_Page' AND t.[PropertyName] = 'Awards' AND i.[Awards] IS NOT NULL
        -- Insert row where property doesn't exist and setting to non-null
        INSERT INTO [dbo].[UserDefinedSingleInstanceProperty] ([TableName], [PropertyName], [RowID], [RowKey], [PropertyStringValue])
            SELECT 'Profile_Page', 'Awards', i.[ContactKey], '00000000-0000-0000-0000-000000000000', i.[Awards]
              FROM inserted i INNER JOIN deleted d ON i.[ContactKey] = d.[ContactKey]
             WHERE i.[Awards] IS NOT NULL AND d.[Awards] IS NULL
                   AND NOT EXISTS (SELECT 1 FROM [dbo].[UserDefinedSingleInstanceProperty] s
                                    WHERE s.[TableName] = 'Profile_Page' AND s.[PropertyName] = 'Awards' AND s.[RowID] = i.[ContactKey])
        -- Delete row if setting to null
        DELETE t
          FROM [dbo].[UserDefinedSingleInstanceProperty] t
               INNER JOIN inserted i ON t.[RowID] = i.[ContactKey]
         WHERE t.[TableName] = 'Profile_Page' AND t.[PropertyName] = 'Awards' AND i.[Awards] IS NULL
    END
