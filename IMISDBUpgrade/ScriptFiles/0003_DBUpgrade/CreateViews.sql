...
AS
     ...
     SELECT i.[Balance]
      ...
      WHERE NOT EXISTS ( SELECT 1 AS Expr1
                         ...
                         FROM   vBoRecurringDonationPaymentAttemptsExceeded

                         WHERE  i.REFERENCE_NUM = vBoRecurringDonationPaymentAttemptsExceeded.InvoiceNumber )
...
AS
SELECT vAutoPayInvoicesFundraising.code_Token AS Code_Token,vAutoPayInvoicesFundraising.Company,vAutoPayInvoicesFundraising.Description,vAutoPayInvoicesFundraising.DonorID,vAutoPayInvoicesFundraising.DUE_DATE,vAutoPayInvoicesFundraising.FullName,vAutoPayInvoicesFundraising.InvoiceNumber,vAutoPayInvoicesFundraising.PaymentAmount
...
SELECT [Code_Token] AS codeToken , [Company],  [Description],  [DUE_DATE] AS [DueDate], [InvoiceNumber] AS [InvoiceNumber], 
  ...
  FROM [dbo].[vBoAutoPayInvoicesDues] 
 UNION ALL 
SELECT [Code_Token] AS codeToken, [Company], [Description], [DUE_DATE] AS [DueDate], [InvoiceNumber] AS [InvoiceNumber], 
  ...
  FROM [dbo].[vBoAutoPayInvoicesFundraising]   
...
SELECT
       ...
       CASE WHEN s.[ID] IS NULL THEN ''
            ...
            WHEN CAST(s.[BILL_DATE] AS date) < CAST(n.[MT_CHANGE_DATE] AS date) THEN n.[PREVIOUS_MT]
            ELSE n.[MEMBER_TYPE]
...
AS
SELECT vAutoPayInstructionDetail.CancellationReason,vAutoPayInstructionDetail.CancelledOn,vAutoPayInstructionDetail.ID,vAutoPayInstructionDetail.PaymentMethodSummary,vAutoPayInstructionDetail.SourceSystem,vAutoPayInstructionDetail.Status,vAutoPayInstructionDetail.InstructionId
...
AS
SELECT ContactSocialNetwork.ContactKey,ContactSocialNetwork.ContactSocialNetworkKey,ContactSocialNetwork.SocialNetworkKey,ContactSocialNetwork.SocialNetworkUserId,ContactSocialNetwork.SocialNetworkUserName
...
AS
  WITH BillingCycles(CycleName,IS_FOR_JOIN,IS_FOR_RENEW) AS (
...
AS
SELECT vSoaItem.Description,vSoaItem.IS_KIT,vSoaItem.IsComponentPriced,vSoaItem.ItemClass,vSoaItem.ItemId,vSoaItem.Name,vSoaItem.ProductCode
  FROM vSoaItem
GO
...
FROM 
	[dbo].[CustomerExperience] 
		INNER JOIN [dbo].[vOfferedCustomerExperience] ON [dbo].[CustomerExperience].[CustomerExperienceKey] = [dbo].[vOfferedCustomerExperience].[OfferedCustomerExperienceKey] 
		INNER JOIN [dbo].[vBoContact] ON [dbo].[CustomerExperience].[ContactKey] = [dbo].[vBoContact].[ContactKey] 
...
FROM 
	[dbo].[CustomerExperience] AS [CustomerExperience1] 
		INNER JOIN [dbo].[vUnofferedCustomerExperience] ON [CustomerExperience1].[CustomerExperienceKey] = [dbo].[vUnofferedCustomerExperience].[UnofferedCustomerExperienceKey] 
		INNER JOIN [dbo].[vBoContact] AS [vBoContact1] ON [CustomerExperience1].[ContactKey] = [vBoContact1].[ContactKey] 
...
FROM 
	[dbo].[CustomerExperience] AS [CustomerExperience2] 
		INNER JOIN [dbo].[vUserDefinedCustomerExperience] ON [CustomerExperience2].[CustomerExperienceKey] = [dbo].[vUserDefinedCustomerExperience].[UserDefinedCustomerExperienceKey] 
		INNER JOIN [dbo].[vBoContact] AS [vBoContact2] ON [CustomerExperience2].[ContactKey] = [vBoContact2].[ContactKey] 
...
FROM
	[dbo].[CustomerExperience]
		INNER JOIN [dbo].[vOfferedCustomerExperience] ON [dbo].[CustomerExperience].[CustomerExperienceKey] = [dbo].[vOfferedCustomerExperience].[OfferedCustomerExperienceKey]
		INNER JOIN [dbo].[vBoNetContact] ON [dbo].[CustomerExperience].[ContactKey] = [dbo].[vBoNetContact].[ContactKey] 
		INNER JOIN [dbo].[CustomerExperienceStatusRef] ON [dbo].[CustomerExperience].[CustomerExperienceStatusCode] = [dbo].[CustomerExperienceStatusRef].[CustomerExperienceStatusCode] 
		LEFT JOIN [dbo].[GradeRef] ON [dbo].[vOfferedCustomerExperience].[OfferedCustomerExperienceGradeKey] = [dbo].[GradeRef].[GradeKey]
...
FROM
	[dbo].[CustomerExperience] AS [CustomerExperience1] 
		INNER JOIN [dbo].[vUnofferedCustomerExperience] ON [CustomerExperience1].[CustomerExperienceKey] = [dbo].[vUnofferedCustomerExperience].[UnofferedCustomerExperienceKey] 
		INNER JOIN [dbo].[vBoNetContact] AS [vBoNetContact1] ON [CustomerExperience1].[ContactKey] = [vBoNetContact1].[ContactKey] 
		INNER JOIN [dbo].[CustomerExperienceStatusRef] AS [CustomerExperienceStatusCodeRef1] ON [CustomerExperience1].[CustomerExperienceStatusCode] = [CustomerExperienceStatusCodeRef1].[CustomerExperienceStatusCode]
...
FROM 
	[dbo].[CustomerExperience] AS [CustomerExperience2] 
		INNER JOIN [dbo].[vUserDefinedCustomerExperience] ON [CustomerExperience2].[CustomerExperienceKey] = [dbo].[vUserDefinedCustomerExperience].[UserDefinedCustomerExperienceKey] 
		INNER JOIN [dbo].[vBoNetContact] AS [vBoNetContact2] ON [CustomerExperience2].[ContactKey] = [vBoNetContact2].[ContactKey] 
		INNER JOIN [dbo].[CustomerExperienceStatusRef] AS [CustomerExperienceStatusCodeRef2] ON [CustomerExperience2].[CustomerExperienceStatusCode] = [CustomerExperienceStatusCodeRef2].[CustomerExperienceStatusCode]
...
SELECT s.RowID AS [ContactKey],
       ...
       CAST(MAX(CASE WHEN s.PropertyName = 'CommunityInvolvement' THEN s.PropertyStringValue ELSE NULL END) AS nvarchar(400)) AS [CommunityInvolvement],
       CAST(MAX(CASE WHEN s.PropertyName = 'Awards' THEN s.PropertyStringValue ELSE NULL END) AS nvarchar(200)) AS [Awards]
  FROM [dbo].[UserDefinedSingleInstanceProperty] s
...
AS
SELECT vUDBOProfile_Page.ContactKey,vUDBOProfile_Page.Type,vUDBOProfile_Page.CompanySize,vUDBOProfile_Page.Founded,vUDBOProfile_Page.Facebook,vUDBOProfile_Page.LinkedIn,vUDBOProfile_Page.Education,vUDBOProfile_Page.PastExperience,vUDBOProfile_Page.CurrentPosition,vUDBOProfile_Page.About,vUDBOProfile_Page.CommunityInvolvement,vUDBOProfile_Page.Awards
...
SELECT bt.[PartyId], bt.[BillToPartyId], bt.[Ranking] AS [Ranking]
  ...
  (
        ...
        SELECT n.[ID] AS [PartyId], 
               ...
               3 AS [Ranking]
          FROM [Name] n 
               INNER JOIN [ContactMain] c ON n.[ID] = c.[SyncContactID]
               LEFT OUTER JOIN [UserRole] u ON c.[ContactKey] = u.[UserKey] 
         WHERE n.[STATUS] = 'A' -- ONLY INCLUDE IF THE PERSON IS AN ACTIVE USER
           ...
           AND u.RoleKey = '4F0F93A9-00EE-40BA-BCBC-99A5BEB21179'
         UNION ALL
...
AS
    SELECT  CAST('ORGRELATIONSHIP-' + [n].[CO_ID] + ':' + [n].[ID] AS VARCHAR(50)) AS [GroupMemberDetailId],
     ...
     WHERE  [n].[CO_ID] != '' 
     UNION
    SELECT  CAST('ORGRELATIONSHIP-' + [n].[CO_ID] + ':' + [n].[ID] + ':CA' AS VARCHAR(50)) AS [GroupMemberDetailId],
            CAST('ORGRELATIONSHIP-' + [n].[CO_ID] + ':' + [n].[ID] AS VARCHAR(50)) AS [GroupMemberId],
            [n].[ID] AS [PartyId],
            CAST(1 AS BIT) AS [IsActive],
            CAST('COMPANYADMINISTRATOR' AS VARCHAR(30)) AS [GroupRoleId],
            CAST('Company Administrator' AS VARCHAR(100)) AS [RoleName],
...
AS
    SELECT  [GroupMemberDetailId],
      ...
      FROM  [dbo].[vSoaGroupMemberDetailSubscription] WITH (NOEXPAND)
    UNION ALL
    SELECT  [GroupMemberDetailId],
            [GroupMemberId],
            [PartyId],
...
AS
    ...
    SELECT DISTINCT
            ...
            NULL AS [JoinDate],
            NULL AS [DropDate]
      FROM  [dbo].[vSoaGroupMemberSummaryOrganization] AS gmso WITH (NOEXPAND)
...
AS
    SELECT  [GroupId],
      ...
      FROM  [dbo].[vSoaGroupSummaryProduct] WITH (NOEXPAND)
    UNION ALL
    SELECT  [GroupId],
            [ParentEntityTypeName],
            [ParentEntityId],
