...
BEGIN
    ...
    BEGIN
        ...
        IF (@publishedOn IS NOT NULL)    
            EXEC [dbo].[asi_AddToPublishQueue] @publishedOn, @publishedByUserKey, 4;
    END
    ELSE IF EXISTS(SELECT 1 FROM deleted WHERE DocumentTypeCode = N'TCT')
    BEGIN
...
BEGIN
    ...
    INSERT INTO [dbo].[PublishQueue]([PublishedOn], [Status], [Type], [ItemCode], [ItemAction], [PublishServerCode])
        SELECT @publishedOn, 0, 0, i.MEETING, 0, psr.[PublishServerCode]
          FROM inserted i
               LEFT OUTER JOIN deleted d ON i.MEETING = d.MEETING
               CROSS JOIN PublishServerRef psr
         WHERE d.MEETING IS NULL  
    -- Add a row to PublishQueue for every Product being updated
    INSERT INTO [dbo].[PublishQueue]([PublishedOn], [Status], [Type], [ItemCode], [ItemAction], [PublishServerCode])
        SELECT @publishedOn, 0, 0, i.MEETING, 1, psr.[PublishServerCode]
          FROM inserted i
               INNER JOIN deleted d ON i.MEETING = d.MEETING
               CROSS JOIN PublishServerRef psr
    -- Add a row to PublishQueue for every Product being deleted
    INSERT INTO [dbo].[PublishQueue]([PublishedOn], [Status], [Type], [ItemCode], [ItemAction], [PublishServerCode])
        SELECT @publishedOn, 0, 0, d.MEETING, 2, psr.[PublishServerCode]
          FROM deleted d
...
BEGIN
    ...
    -- Grab some data we need, including an 'updated by user' key
    SELECT @entityKey = SystemEntityKey FROM [dbo].[SystemEntity] WHERE [SystemKeyword] = 'Organization'
    ...
    IF NOT EXISTS (SELECT 1 FROM UserMain WHERE UserKey = @adminUserKey)
        SELECT @adminUserKey = UserKey FROM UserMain WHERE UserId = 'SYSTEM';
    IF @adminUserKey IS NULL
        SELECT @adminUserKey = UserKey FROM UserMain WHERE UserId = 'MANAGER';
    SET @now = GETDATE()

    -- Mark any associated 'User' record as disabled
    UPDATE um
       SET um.[IsDisabled] = 1,
           um.[ContactMaster] = '',
           um.[UserId] = ''
    ...
    UPDATE cm
       SET cm.[ContactStatusCode] = cs.[ContactStatusCode],
           cm.[UpdatedByUserKey] = @adminUserKey,
           cm.[UpdatedOn] = @now,
           cm.[SyncContactID] = NULL  -- Have to remove SyncContactID since C/S can reuse IDs
      FROM deleted d 
           INNER JOIN [dbo].[ContactMain] cm ON d.[ID] = cm.[SyncContactID]
           INNER JOIN [dbo].[ContactStatusRef] cs ON cs.[ContactStatusDesc] = 'Delete' AND cs.[IsSystem] = 1
...
BEGIN
    ...
    -- Add a row to PublishQueue for every Product being inserted as new
    INSERT INTO [dbo].[PublishQueue]([PublishedOn], [Status], [Type], [ItemCode], [ItemAction], [PublishServerCode])
        SELECT @publishedOn, 0, 1, i.PRODUCT_CODE, 0, psr.[PublishServerCode]
          FROM inserted i
    ...
    -- Add a row to PublishQueue for every Product being updated
    INSERT INTO [dbo].[PublishQueue]([PublishedOn], [Status], [Type], [ItemCode], [ItemAction], [PublishServerCode])
        SELECT @publishedOn, 0, 1, i.PRODUCT_CODE, 1, psr.[PublishServerCode]
          FROM inserted i
    ...
    -- Add a row to PublishQueue for every Product being deleted
    INSERT INTO [dbo].[PublishQueue]([PublishedOn], [Status], [Type], [ItemCode], [ItemAction], [PublishServerCode])
        SELECT @publishedOn, 0, 1, d.PRODUCT_CODE, 2, psr.[PublishServerCode]
          FROM deleted d
...
BEGIN
    ...
    [CC_NUMBER] varchar(25) COLLATE DATABASE_DEFAULT NOT NULL, [CC_EXPIRE] varchar(10) COLLATE DATABASE_DEFAULT NOT NULL, [CC_AUTHORIZE] varchar(10) COLLATE DATABASE_DEFAULT NOT NULL,
    ...
    [DO_NOT_RECEIPT] tinyint NOT NULL, [CC_STATUS] varchar(1) COLLATE DATABASE_DEFAULT NOT NULL, [ENCRYPT_CC_NUMBER] varchar(100) COLLATE DATABASE_DEFAULT NOT NULL,
    [ENCRYPT_CC_EXPIRE] varchar(100) COLLATE DATABASE_DEFAULT NOT NULL, [FR_ACTIVITY] varchar(1) COLLATE DATABASE_DEFAULT NOT NULL, [FR_ACTIVITY_SEQN] int NOT NULL,
    ...
    [ENCRYPT_CSC] varchar(100) COLLATE DATABASE_DEFAULT NOT NULL, [ISSUE_DATE] varchar(10) COLLATE DATABASE_DEFAULT NOT NULL, [ISSUE_NUMBER] varchar(2) COLLATE DATABASE_DEFAULT NOT NULL,
