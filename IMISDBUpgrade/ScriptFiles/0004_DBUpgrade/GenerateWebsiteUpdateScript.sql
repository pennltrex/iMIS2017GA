...
SET @sql6 = '"'');
SET @xml.modify(''declare namespace w="http://schemas.imis.com/2008/01/DataContracts/Website"; replace value of (/w:Website/w:IsActive/text())[1] with "';
SET @sql7 = '"'');
-- Put the updated blob back
UPDATE [dbo].[DocumentMain]
   SET [Blob] = CAST(CAST(@xml AS varchar(max)) AS image)
 WHERE [DocumentVersionKey] = ''';
SET @sql8 = ''' AND [DocumentStatusCode] = 40 AND [DocumentTypeCode] = ''WEB'';
GO
';
...
SELECT @sql1 + CAST([DocumentVersionKey] as varchar(40)) + 
       ...
       @sql6 + CAST(CAST(SUBSTRING(Blob, 1, DATALENGTH(Blob)) AS varchar(max)) AS xml).value( 'declare namespace w="http://schemas.imis.com/2008/01/DataContracts/Website"; (w:Website/w:IsActive)[1]', 'varchar(256)') + 
       @sql7 + CAST([DocumentVersionKey] as varchar(40)) + @sql8
  FROM [dbo].[DocumentMain] 
