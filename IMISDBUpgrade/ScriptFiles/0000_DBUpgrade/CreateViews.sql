...
IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[vContactCommunicationLogReport]'))
    DROP VIEW [dbo].[vContactCommunicationLogReport]
GO
/* 
This view is used in correspondence management to consolidate inbound and outbound
communication logs together. OutboundCommunicationLog is queried twice,
one has sender's contact key in CommunicationContactKey and the other has
recipient's contact key CommunicationContactKey.
*/
CREATE  VIEW [dbo].[vContactCommunicationLogReport] As
SELECT 
        a.OutboundCommunicationLogKey AS CommunicationLogKey,
        c.DocumentName CommunicationName, 
        a.SentOn AS CommunicationDate, 
        1 AS OutboundLogFlag,
        a.IssueID, 
        b.SourceCodeKey, 
        a.OutboundCommunicationStatusCode,
        d.ContactKey AS SenderContactKey,
        a.ContactKey AS RecipientContactKey,
        a.AddressCategoryCode,
        d.ContactKey AS CommunicationContactKey
  FROM  OutboundCommunicationLog a, PackageLog b, DocumentMain c,
        CorrespondenceSender d
 WHERE a.PackageLogKey = b.PackageLogKey
   AND b.PackageKey = c.DocumentVersionKey
   AND b.CorrespondenceSenderKey = d.CorrespondenceSenderKey
UNION
SELECT 
        a.OutboundCommunicationLogKey,
        c.DocumentName, 
        a.SentOn, 
        1,
        a.IssueID, 
...
SELECT 
        ...
        a.AddressCategoryCode,
        a.SenderContactKey
  FROM  InboundCommunicationLog a
GO
...
IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[vBoContactCommunicationLogReport]'))
    DROP VIEW [dbo].[vBoContactCommunicationLogReport]
GO
CREATE VIEW [dbo].[vBoContactCommunicationLogReport] AS SELECT vContactCommunicationLogReport.CommunicationContactKey,vContactCommunicationLogReport.CommunicationDate,vContactCommunicationLogReport.CommunicationLogKey,vContactCommunicationLogReport.CommunicationName,vContactCommunicationLogReport.IssueID,vContactCommunicationLogReport.OutboundCommunicationStatusCode,vContactCommunicationLogReport.OutboundLogFlag,vContactCommunicationLogReport.RecipientContactKey,vContactCommunicationLogReport.SenderContactKey,vContactCommunicationLogReport.SourceCodeKey,vContactCommunicationLogReport.AddressCategoryCode FROM vContactCommunicationLogReport
GO
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
...
IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[vBoCorrespondenceDistributionLetterTemplateTypeRef]'))
    DROP VIEW [dbo].[vBoCorrespondenceDistributionLetterTemplateTypeRef]
GO
CREATE VIEW [dbo].[vBoCorrespondenceDistributionLetterTemplateTypeRef] AS SELECT CorrespondenceDistributionLetterTemplateTypeRef.CorrespondenceDistributionKey,CorrespondenceDistributionLetterTemplateTypeRef.LetterTemplateTypeDocumentTypeCode FROM CorrespondenceDistributionLetterTemplateTypeRef
GO
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
...
IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[vBoCorrespondenceDistributionRef]'))
    DROP VIEW [dbo].[vBoCorrespondenceDistributionRef]
GO
CREATE VIEW [dbo].[vBoCorrespondenceDistributionRef] AS SELECT CorrespondenceDistributionRef.AddressCategoryCode,CorrespondenceDistributionRef.CorrespondenceDistributionDesc AS Description,CorrespondenceDistributionRef.CorrespondenceDistributionKey,CorrespondenceDistributionRef.IsSystem,CorrespondenceDistributionRef.MethodName,CorrespondenceDistributionRef.SettingsAssemblyName,CorrespondenceDistributionRef.SettingsTypeName,CorrespondenceDistributionRef.SettingsControlPath FROM CorrespondenceDistributionRef
GO
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
...
IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[vBoCorrespondenceSender]'))
    DROP VIEW [dbo].[vBoCorrespondenceSender]
GO
CREATE VIEW [dbo].[vBoCorrespondenceSender] AS SELECT CorrespondenceSender.CorrespondenceSenderStatusCode,CorrespondenceSender.ContactKey,CorrespondenceSender.CreatedOn,CorrespondenceSender.EmailAccountKey,CorrespondenceSender.FaxAccountKey,CorrespondenceSender.UpdatedOn,CorrespondenceSender.CorrespondenceSenderKey FROM CorrespondenceSender
GO
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
...
IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[vBoCorrespondenceSenderStatusRef]'))
    DROP VIEW [dbo].[vBoCorrespondenceSenderStatusRef]
GO
CREATE VIEW [dbo].[vBoCorrespondenceSenderStatusRef] AS SELECT CorrespondenceSenderStatusRef.CorrespondenceSenderStatusCode AS Code,CorrespondenceSenderStatusRef.CorrespondenceSenderStatusDesc AS Description,CorrespondenceSenderStatusRef.CorrespondenceSenderStatusName AS Name FROM CorrespondenceSenderStatusRef
GO
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
...
IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[vBoCorrespondenceSignature]'))
    DROP VIEW [dbo].[vBoCorrespondenceSignature]
GO
CREATE VIEW [dbo].[vBoCorrespondenceSignature] AS SELECT CorrespondenceSignature.Closing,CorrespondenceSignature.CorrespondenceSenderKey,CorrespondenceSignature.CorrespondenceSignatureKey,CorrespondenceSignature.Description,CorrespondenceSignature.Name,CorrespondenceSignature.SalutationKey,CorrespondenceSignature.Signature,CorrespondenceSignature.SignatureImage,CorrespondenceSignature.IsPreferred,CorrespondenceSignature.LetterheadKey,CorrespondenceSignature.MimeType FROM CorrespondenceSignature
GO
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
...
GO
CREATE VIEW [dbo].[vBoCsProduct] AS SELECT Product.DEFERRED_INCOME_ACCOUNT AS DeferredIncomeAccount,Product.DESCRIPTION AS Description,Product.INCOME_ACCOUNT AS IncomeAccount,Product.PRICE_1 AS Price1,Product.PRICE_2 AS Price2,Product.PRICE_3 AS Price3,Product.PROD_TYPE AS ProductType,Product.PRODUCT_CODE AS ProductCode,Product.PRODUCT_MAJOR AS ProductMajor,Product.PRODUCT_MINOR AS ProductMinor,Product.STATUS AS Status,Product.TITLE AS Title,Product.TITLE_KEY AS TitleKey,Product.CATEGORY AS ProductCategory,Product.APPLY_IMAGE,Product.APPLY_THUMBNAIL,Product.IMAGE_URL,Product.THUMBNAIL_URL,Product.WEB_DESC,Product.WEB_OPTION FROM Product
GO
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
...
IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[vBoEmailAccountRef]'))
    DROP VIEW [dbo].[vBoEmailAccountRef]
GO
CREATE VIEW [dbo].[vBoEmailAccountRef] AS SELECT EmailAccountRef.EmailAccountKey,EmailAccountRef.EmailAddress,EmailAccountRef.FriendlyName,EmailAccountRef.Password,EmailAccountRef.UserName,EmailAccountRef.EmailAccountDesc AS Description,EmailAccountRef.ReplyToEmailAddress,EmailAccountRef.ReplyToFriendlyName,EmailAccountRef.EmailServerName FROM EmailAccountRef
GO
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
...
IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[vBoFaxAccountRef]'))
    DROP VIEW [dbo].[vBoFaxAccountRef]
GO
CREATE VIEW [dbo].[vBoFaxAccountRef] AS SELECT FaxAccountRef.FaxAccountDesc AS Description,FaxAccountRef.FaxAccountKey,FaxAccountRef.FaxNumber,FaxAccountRef.FaxServerName,FaxAccountRef.FriendlyName,FaxAccountRef.Password,FaxAccountRef.UserName FROM FaxAccountRef
GO
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
...
IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[vFullAddressCorrespondenceReport]'))
    DROP VIEW [dbo].[vFullAddressCorrespondenceReport]
GO
/*
This view is used in correspondence management for reporting FullAddress. 
The results are all the possible combination of FullAddress,CommunicationReason and Salutation.
Columns include the regenerated FullAddressText, 
the salutation text to use, and the referenced Address and PhysicalAddress data.
*/
CREATE  VIEW [dbo].[vFullAddressCorrespondenceReport]
AS 
  SELECT fa.FullAddressKey,
         fa.ContactKey,
         crr.CommunicationReasonKey,
         sr.SalutationKey,
         dbo.asi_GetFullAddressText(fa.FullAddressKey,fa.ContactKey,fa.PhysicalAddressKey,sr.SalutationKey,crr.Formula) AS FullAddressText,
         CASE WHEN fa.IsAddresseeOverridden = 1 THEN fa.AddresseeText
               ELSE cs.SalutationText END AS SalutationText,
         fa.AddressPurposeKey,
         fa.AdditionalLine1,
         fa.AdditionalLine2,
         pam.BadAddressReasonKey AS PhysicalBadAddressReasonKey,
         pa.Address1,
         pa.Address2,
         pa.Address3,
         pa.City,
         ...
         pa.CountryCode,
         pa.County,
         dpam.FormattedAddress AS DaytimePhoneFormattedAddress,
         dpam.BadAddressReasonKey AS DaytimePhoneBadAddressReasonKey,
         epam.FormattedAddress AS EveningPhoneFormattedAddress,
         epam.BadAddressReasonKey AS EveningPhoneBadAddressReasonKey,
         fam.FormattedAddress AS FaxFormattedAddress,
         fam.BadAddressReasonKey AS FaxBadAddressReasonKey,
         eam.FormattedAddress AS EmailFormattedAddress,
         eam.BadAddressReasonKey AS EmailBadAddressReasonKey
   FROM FullAddress fa 
   LEFT OUTER JOIN AddressMain pam
     ON fa.PhysicalAddressKey = pam.AddressKey
   LEFT OUTER JOIN PhysicalAddress pa
     ON pam.AddressKey = pa.AddressKey
   LEFT OUTER JOIN AddressMain dpam
     ON fa.DaytimePhoneAddressKey = dpam.AddressKey
   LEFT OUTER JOIN AddressMain epam
     ON fa.EveningPhoneAddressKey = epam.AddressKey
   LEFT OUTER JOIN AddressMain fam
     ON fa.FaxAddressKey = fam.AddressKey
   LEFT OUTER JOIN AddressMain eam
     ON fa.EmailAddressKey = eam.AddressKey
   CROSS JOIN SalutationRef sr
   LEFT OUTER JOIN ContactSalutation cs
     ON fa.ContactKey = cs.ContactKey
    AND cs.SalutationKey = sr.SalutationKey
        ,CommunicationReasonRef crr
GO
...
IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[vBoFullAddressCorrespondenceReport]'))
    DROP VIEW [dbo].[vBoFullAddressCorrespondenceReport]
GO
CREATE VIEW [dbo].[vBoFullAddressCorrespondenceReport] AS SELECT vFullAddressCorrespondenceReport.AdditionalLine1,vFullAddressCorrespondenceReport.AdditionalLine2,vFullAddressCorrespondenceReport.Address1,vFullAddressCorrespondenceReport.Address2,vFullAddressCorrespondenceReport.Address3,vFullAddressCorrespondenceReport.AddressPurposeKey,vFullAddressCorrespondenceReport.City,vFullAddressCorrespondenceReport.CommunicationReasonKey,vFullAddressCorrespondenceReport.CountryCode,vFullAddressCorrespondenceReport.County,vFullAddressCorrespondenceReport.DaytimePhoneBadAddressReasonKey,vFullAddressCorrespondenceReport.DaytimePhoneFormattedAddress,vFullAddressCorrespondenceReport.EmailBadAddressReasonKey,vFullAddressCorrespondenceReport.EmailFormattedAddress,vFullAddressCorrespondenceReport.EveningPhoneBadAddressReasonKey,vFullAddressCorrespondenceReport.EveningPhoneFormattedAddress,vFullAddressCorrespondenceReport.FaxBadAddressReasonKey,vFullAddressCorrespondenceReport.FaxFormattedAddress,vFullAddressCorrespondenceReport.FullAddressKey,vFullAddressCorrespondenceReport.FullAddressText,vFullAddressCorrespondenceReport.PhysicalBadAddressReasonKey,vFullAddressCorrespondenceReport.PostalCode,vFullAddressCorrespondenceReport.Region,vFullAddressCorrespondenceReport.SalutationKey,vFullAddressCorrespondenceReport.SalutationText,vFullAddressCorrespondenceReport.StateProvinceCode,vFullAddressCorrespondenceReport.ContactKey FROM vFullAddressCorrespondenceReport
GO
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
...
GO
CREATE VIEW [dbo].[vBoGLExport] AS SELECT GLExport.BeginDate,GLExport.CreatedByUserKey,GLExport.CreatedOn,GLExport.Description,GLExport.EndDate,GLExport.GLExportKey,GLExport.GLExportSummaryLevelCode,GLExport.Name,GLExport.RunNumber,GLExport.SystemEntityKey,GLExport.UpdatedByUserKey,GLExport.UpdatedOn,GLExport.FinancialEntityKey FROM GLExport
GO
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
...
IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[vBoGroupTool]'))
    DROP VIEW [dbo].[vBoGroupTool]
GO
CREATE VIEW [dbo].[vBoGroupTool] AS SELECT GroupToolRef.GroupToolDesc,GroupToolRef.GroupToolKey FROM GroupToolRef
GO
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
...
IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[vBoInboundCommunicationLog]'))
    DROP VIEW [dbo].[vBoInboundCommunicationLog]
GO
CREATE VIEW [dbo].[vBoInboundCommunicationLog] AS SELECT InboundCommunicationLog.AddressCategoryCode,InboundCommunicationLog.ContentImage,InboundCommunicationLog.ContentText,InboundCommunicationLog.Description,InboundCommunicationLog.EmailAddressKey,InboundCommunicationLog.FaxAddressKey,InboundCommunicationLog.InboundCommunicationLogKey,InboundCommunicationLog.IssueID,InboundCommunicationLog.PhysicalAddressKey,InboundCommunicationLog.ReceivedOn,InboundCommunicationLog.SourceCodeKey,InboundCommunicationLog.AccessKey,InboundCommunicationLog.SenderContactKey,InboundCommunicationLog.Subject,InboundCommunicationLog.ContentImageMimeType FROM InboundCommunicationLog
GO
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
...
IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[vBoLetterheadRef]'))
    DROP VIEW [dbo].[vBoLetterheadRef]
GO
CREATE VIEW [dbo].[vBoLetterheadRef] AS SELECT LetterheadRef.BottomMargin,LetterheadRef.LeftMargin,LetterheadRef.LetterheadDesc AS Description,LetterheadRef.LetterheadKey,LetterheadRef.PageHeight,LetterheadRef.PageWidth,LetterheadRef.PaperSizeKey,LetterheadRef.PrinterKey,LetterheadRef.RightMargin,LetterheadRef.TopMargin,LetterheadRef.VerticalAlignmentCode,LetterheadRef.IsPortrait,LetterheadRef.FirstPagePaperSourceKey,LetterheadRef.OtherPagesPaperSourceKey FROM LetterheadRef
GO
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
...
IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[vBoLetterTemplateTypeDocumentTypeRef]'))
    DROP VIEW [dbo].[vBoLetterTemplateTypeDocumentTypeRef]
GO
CREATE VIEW [dbo].[vBoLetterTemplateTypeDocumentTypeRef] AS SELECT LetterTemplateTypeDocumentTypeRef.DocumentTypeCode,LetterTemplateTypeDocumentTypeRef.LetterTemplateTypeDocumentTypeCode FROM LetterTemplateTypeDocumentTypeRef
GO
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
...
IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[vBoLetterTemplateTypeRef]'))
    DROP VIEW [dbo].[vBoLetterTemplateTypeRef]
GO
CREATE VIEW [dbo].[vBoLetterTemplateTypeRef] AS SELECT LetterTemplateTypeRef.AssemblyName,LetterTemplateTypeRef.TypeName,LetterTemplateTypeRef.DocumentTypeCode FROM LetterTemplateTypeRef
GO
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
...
IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[vBoMonitoredEmailAccountRef]'))
    DROP VIEW [dbo].[vBoMonitoredEmailAccountRef]
GO
CREATE VIEW [dbo].[vBoMonitoredEmailAccountRef] AS SELECT MonitoredEmailAccountRef.EmailServerName,MonitoredEmailAccountRef.MonitoredEmailAccountKey,MonitoredEmailAccountRef.Password,MonitoredEmailAccountRef.UserName,MonitoredEmailAccountRef.MonitoredEmailAccountName AS Name,MonitoredEmailAccountRef.InboundEmailProcessingWorkflowKey FROM MonitoredEmailAccountRef
GO
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
...
IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[vBoOutboundCommunicationLog]'))
    DROP VIEW [dbo].[vBoOutboundCommunicationLog]
GO
CREATE VIEW [dbo].[vBoOutboundCommunicationLog] AS SELECT OutboundCommunicationLog.AddressCategoryCode,OutboundCommunicationLog.AddressKey,OutboundCommunicationLog.AddressText,OutboundCommunicationLog.ContactKey,OutboundCommunicationLog.IssueID,OutboundCommunicationLog.OutboundCommunicationLogKey,OutboundCommunicationLog.OutboundCommunicationStatusCode,OutboundCommunicationLog.OutboundCommunicationStatusReasonKey,OutboundCommunicationLog.PackageLogKey,OutboundCommunicationLog.SentOn FROM OutboundCommunicationLog
GO
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
...
IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[vBoOutboundCommunicationStatusReasonRef]'))
    DROP VIEW [dbo].[vBoOutboundCommunicationStatusReasonRef]
GO
CREATE VIEW [dbo].[vBoOutboundCommunicationStatusReasonRef] AS SELECT OutboundCommunicationStatusReasonRef.OutboundCommunicationStatusReasonKey,OutboundCommunicationStatusReasonRef.OutboundCommunicationStatusReasonDesc AS Description,OutboundCommunicationStatusReasonRef.IsSystem FROM OutboundCommunicationStatusReasonRef
GO
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
...
IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[vBoOutboundLetterTemplateLog]'))
    DROP VIEW [dbo].[vBoOutboundLetterTemplateLog]
GO
CREATE VIEW [dbo].[vBoOutboundLetterTemplateLog] AS SELECT OutboundLetterTemplateLog.Letter,OutboundLetterTemplateLog.LetterComponentKey,OutboundLetterTemplateLog.LetterTemplateKey,OutboundLetterTemplateLog.OutboundCommunicationLogKey,OutboundLetterTemplateLog.OutboundLetterTemplateLogKey,OutboundLetterTemplateLog.FileTypeCode,OutboundLetterTemplateLog.Subject FROM OutboundLetterTemplateLog
GO
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
...
IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[vBoPackageLog]'))
    DROP VIEW [dbo].[vBoPackageLog]
GO
CREATE VIEW [dbo].[vBoPackageLog] AS SELECT PackageLog.CorrespondenceSenderKey,PackageLog.CreatedOn,PackageLog.PackageKey,PackageLog.PackageLogKey,PackageLog.SourceCodeKey,PackageLog.UpdatedOn,PackageLog.Cost,PackageLog.PackageLogStatusCode,PackageLog.QueryParameterValues,PackageLog.CreatedByUserKey,PackageLog.ReturnedCount,PackageLog.SentCount FROM PackageLog
GO
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
...
IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[vBoPackageLogPackageQueue]'))
    DROP VIEW [dbo].[vBoPackageLogPackageQueue]
GO
CREATE VIEW [dbo].[vBoPackageLogPackageQueue] AS SELECT PackageLogPackageQueue.CreatedOn,PackageLogPackageQueue.PackageLogKey,PackageLogPackageQueue.PackageQueueKey FROM PackageLogPackageQueue
GO
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
...
IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[vBoPackageLogSupplement]'))
    DROP VIEW [dbo].[vBoPackageLogSupplement]
GO
CREATE VIEW [dbo].[vBoPackageLogSupplement] AS SELECT PackageLogSupplement.IsEmailAttachment,PackageLogSupplement.IsEnclosure,PackageLogSupplement.PackageLogKey,PackageLogSupplement.SupplementKey FROM PackageLogSupplement
GO
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
...
IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[vBoPaperSourceRef]'))
    DROP VIEW [dbo].[vBoPaperSourceRef]
GO
CREATE VIEW [dbo].[vBoPaperSourceRef] AS SELECT PaperSourceRef.IsVisible,PaperSourceRef.PaperSourceKey,PaperSourceRef.PaperSourceName AS Name,PaperSourceRef.PrinterKey FROM PaperSourceRef
GO
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
...
IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[vBoPrinterRef]'))
    DROP VIEW [dbo].[vBoPrinterRef]
GO
CREATE VIEW [dbo].[vBoPrinterRef] AS SELECT PrinterRef.PrinterDesc AS Description,PrinterRef.PrinterKey,PrinterRef.PrinterPath FROM PrinterRef
GO
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
...
GO
CREATE VIEW [dbo].[vBoUserDefinedField] AS SELECT UserDefinedFieldRef.IsRequired,UserDefinedFieldRef.PropertyDefinitionKey,UserDefinedFieldRef.SortOrder,UserDefinedFieldRef.SystemEntityKey,UserDefinedFieldRef.UseInSearchFlag,UserDefinedFieldRef.UserDefinedFieldKey,UserDefinedFieldRef.UserDefinedFieldName AS Name FROM UserDefinedFieldRef
GO
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
...
IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[vBoWebPartGalleryEntry]'))
    DROP VIEW [dbo].[vBoWebPartGalleryEntry]
GO
CREATE VIEW [dbo].[vBoWebPartGalleryEntry] AS SELECT WebPartGalleryEntryRef.WebPartDescription,WebPartGalleryEntryRef.WebPartGalleryEntryKey,WebPartGalleryEntryRef.WebPartGalleryKey,WebPartGalleryEntryRef.WebPartName,WebPartGalleryEntryRef.WebPartTypeName,WebPartGalleryEntryRef.WebUserControlPath FROM WebPartGalleryEntryRef
GO
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
...
IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[vContactCorrespondencePreferencesReport]'))
    DROP VIEW [dbo].[vContactCorrespondencePreferencesReport]
GO
/*
This view is a reporting view for preference information for correspondence receipients.
The resultset is all the possible combination of Contacts (that have at least one address and
thus an AddressToUse) and CommunicationReason.  The columns include overall preference 
information, preference information for all the existing communication reason, and preference 
information on mail, phone, fax and email communication.
*/
CREATE  VIEW [dbo].[vContactCorrespondencePreferencesReport]
AS
SELECT cm.ContactKey, 
       crr.CommunicationReasonKey,
       cm.NoSolicitationFlag AS OverallNoSolicitationFlag,
       cm.PreferredAddressCategoryCode AS OverallPreferredAddressCategoryCode,
       cm.TextOnlyEmailFlag,
       atu.FullAddressKey,
       crr.IsSolicitation,
       ccrp.NoCommunicationByMailFlag AS CommReasonNoMailFlag,
       ccrp.NoCommunicationByPhoneFlag AS CommReasonNoPhoneFlag,
       ccrp.NoCommunicationByFaxFlag AS CommReasonNoFaxFlag,
       ccrp.NoCommunicationByEmailFlag AS CommReasonNoEmailFlag,
       ccrp.PreferredAddressCategoryCode AS CommReasonPreferredAddressCategoryCode,
       cacpr.MailNoSolicitationFlag,
       cacpr.MailOptOutFlag,
       cacpr.PhoneNoSolicitationFlag,
       cacpr.PhoneOptOutFlag,
       cacpr.FaxNoSolicitationFlag,
       cacpr.FaxOptOutFlag,
       cacpr.EmailNoSolicitationFlag,
       cacpr.EmailOptOutFlag
  FROM ContactMain cm CROSS JOIN CommunicationReasonRef crr
       INNER JOIN AddressToUse atu
       ON cm.ContactKey = atu.ContactKey
       AND crr.CommunicationReasonKey = atu.CommunicationReasonKey
       INNER JOIN FullAddress fa
       ON atu.FullAddressKey = fa.FullAddressKey
       LEFT OUTER JOIN ContactCommunicationReasonPreferences ccrp
       ON fa.ContactKey = ccrp.ContactKey
       AND crr.CommunicationReasonKey = ccrp.CommunicationReasonKey
       INNER JOIN vContactAddressCategoryPreferencesReport cacpr
       ON cm.ContactKey = cacpr.ContactKey
UNION
SELECT cm.ContactKey, 
       crr1.CommunicationReasonKey,
       cm.NoSolicitationFlag AS OverallNoSolicitationFlag,
       cm.PreferredAddressCategoryCode AS OverallPreferredAddressCategoryCode,
       cm.TextOnlyEmailFlag,
       atu2.FullAddressKey,
       ...
       cacpr.EmailNoSolicitationFlag,
       cacpr.EmailOptOutFlag
  FROM ContactMain cm CROSS JOIN CommunicationReasonRef crr1
       LEFT OUTER JOIN ContactCommunicationReasonPreferences ccrp
       ON cm.ContactKey = ccrp.ContactKey
       AND ccrp.CommunicationReasonKey = crr1.CommunicationReasonKey
       LEFT OUTER JOIN AddressToUse atu1
       ON cm.ContactKey = atu1.ContactKey
       AND crr1.CommunicationReasonKey = atu1.CommunicationReasonKey
       INNER JOIN AddressToUse atu2
       ON cm.ContactKey = atu2.ContactKey
       INNER JOIN CommunicationReasonRef crr2
       ON atu2.CommunicationReasonKey = crr2.CommunicationReasonKey
       AND crr2.IsDefaultType = 1
       AND cm.ContactKey = atu2.ContactKey
       INNER JOIN vContactAddressCategoryPreferencesReport cacpr
       ON cm.ContactKey = cacpr.ContactKey
 WHERE atu1.FullAddressKey IS NULL
GO
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
...
GO
CREATE VIEW [vSoaEvent]
AS  
SELECT 'EVENT-' + [Meet_Master].[MEETING] AS [EventId],
...
GO
CREATE VIEW [vSoaEventFunction]
AS  
SELECT 'EVENT-' + [p].[PRODUCT_MAJOR] AS [EventId],
       ...
       CAST('' AS VARCHAR (100)) AS [FacilityName]
  FROM [Product] [p] 
       INNER JOIN [Product_Function] [f] ON [p].[PRODUCT_CODE] = [f].[PRODUCT_CODE]
...
GO
CREATE VIEW [vSoaGeneralReference]
AS
SELECT CAST([Gen_Tables].[TABLE_NAME] + ':' + [Gen_Tables].[CODE] AS NVARCHAR(1000)) AS [GeneralReferenceId],
       [Gen_Tables].[TABLE_NAME] AS [Table],
       [Gen_Tables].[CODE] AS [Name],
       CAST([Gen_Tables].[DESCRIPTION] AS NVARCHAR(1000)) AS [Description]
  FROM [dbo].[Gen_Tables]
...
GO
CREATE VIEW [vSoaGroupMemberDetail]
AS  
SELECT CAST([p1].[PRODUCT_CODE] + ':' + [s].[ID] AS VARCHAR(50)) AS [GroupMemberDetailId],
       ...
       CAST(CASE WHEN ( [s].[PAID_THRU] IS NULL
                        OR [s].[PAID_THRU] > DATEADD(day, 1, GETDATE()) )
                      AND ( [s].[BEGIN_DATE] IS NULL
                            OR [s].[BEGIN_DATE] < GETDATE() ) THEN 1
                 ELSE 0
            END AS BIT) AS [IsActive],
       CAST(NULL AS VARCHAR(20)) AS [GroupRoleId],
       NULL AS [RoleName],
       NULL AS [RoleDescription],
       NULL AS [RolePriority]
  FROM [dbo].[Subscriptions] AS [s]
       INNER JOIN [dbo].[Product] AS [p1] ON [s].[PRODUCT_CODE] = [p1].[PRODUCT_CODE]
 WHERE ( [p1].[PROD_TYPE] IN ( 'DUES', 'SUBS', 'SEC', 'CHAPT', 'VOL', 'MISC' ) )
UNION ALL
SELECT CAST('seq:' + CAST([a].[SEQN] AS VARCHAR(10)) AS VARCHAR(50)) AS [GroupMemberDetailId],
       CAST([p2].[PRODUCT_CODE] + ':' + [a].[ID] AS VARCHAR(50)) AS [GroupMemberId],
       [a].[ID] AS [PartyId],
       [a].[EFFECTIVE_DATE] AS [EffectiveDate],
       [a].[THRU_DATE] AS [ExpirationDate],
       CAST(CASE WHEN ( [a].[THRU_DATE] IS NULL
                        OR [a].[THRU_DATE] > DATEADD(day, 1, GETDATE()) )
                      AND ( [a].[EFFECTIVE_DATE] IS NULL
                            OR [a].[EFFECTIVE_DATE] < GETDATE() ) THEN 1
                 ELSE 0
            END AS BIT) AS [IsActive],
       CASE WHEN [cp].[POSITION_CODE] IS NOT NULL THEN CAST('COMMITTEE:' + [cp].[POSITION_CODE] AS VARCHAR(20))
            ELSE CAST(NULL AS VARCHAR(20))
       END AS [GroupRoleId],
       [cp].[POSITION_CODE] AS [RoleName],
       [cp].[TITLE] AS [RoleDescription],
...
GO
CREATE VIEW [vSoaGroupMemberSummary]
AS  
SELECT CAST([p].[PRODUCT_CODE] + ':' + [s].[ID] AS VARCHAR(50)) AS [GroupMemberId],
       CAST([p].[PRODUCT_CODE] AS VARCHAR(50)) AS [GroupId],
       'Public' AS [ParentEntityTypeName],
       'Public Groups' AS [ParentEntityId],
       [p].[PROD_TYPE] AS [GroupType],
       [p].[TITLE] AS [GroupName],
       CAST([p].[DESCRIPTION] AS VARCHAR(6000)) AS [GroupDescription],
       CAST(0 AS BIT) AS [IsSimpleGroup],
       CAST(0 AS BIT) AS [IsMemberGroup],
       [ps].[PartyId],
       [ps].[Id],
       [ps].[Status],
       [ps].[Name] AS [PartyName],
       [ps].[Sort] AS [PartySort],
       [ps].[CityName],
       [ps].[CountrySubEntityName],
       [ps].[CountryName],
       [ps].[Phone],
       [ps].[Email],
       [ps].[IsMarkedForDelete],
       CAST(0 AS BIT) [IsPrimary],
       CASE WHEN ( [s].[PAID_THRU] IS NULL
                   OR [s].[PAID_THRU] > DATEADD(day, 1, GETDATE()) )
                 AND ( [s].[BEGIN_DATE] IS NULL
                       OR [s].[BEGIN_DATE] < GETDATE() ) THEN CAST(1 AS BIT)
            ELSE CAST(0 AS BIT)
       END AS [IsActive],
  ...
  FROM [dbo].[Subscriptions] AS [s]
       ...
       INNER JOIN [dbo].[vSoaPartySummary] AS [ps] ON [s].[ID] = [ps].[PartyId]
 WHERE ([p].[PROD_TYPE] IN ( 'DUES', 'SUBS', 'SEC', 'CHAPT', 'VOL'))
UNION ALL
SELECT DISTINCT CAST([p].[PRODUCT_CODE] + ':' + [a].[ID] AS VARCHAR(50)) AS [GroupMemberId],
       CAST([p].[PRODUCT_CODE] AS VARCHAR(50)) AS [GroupId],
       'Public' AS [ParentEntityTypeName],
       'Public Groups' AS [ParentEntityId],
       [p].[PROD_TYPE] AS [GroupType],
  ...
  FROM [dbo].[Activity] AS [a]
       ...
       INNER JOIN [dbo].[vSoaPartySummary] [ps] ON [a].[ID] = [ps].[PartyId]
       LEFT OUTER JOIN [dbo].[Committee_Position] AS [cp] ON [a].[ACTION_CODES] = [cp].[POSITION_CODE]
                                                             AND [a].[ACTION_CODES] != ''
...
SELECT CAST('EVENT-' + [mm].[MEETING] + ':' + [o].[ST_ID] AS VARCHAR(50)) AS GroupMemberId,
       ...
       'Public Groups' AS [ParentEntityId],
       'EVENT' AS [GroupType],
       [mm].[TITLE] AS [GroupName],
       CAST([mm].[DESCRIPTION] AS VARCHAR(6000)) AS [GroupDescription],
 ...
 FROM [dbo].[Meet_Master] AS [mm]
      ...
      INNER JOIN [dbo].[vSoaPartySummary] AS [ps] ON [o].[ST_ID] = [ps].[PartyId]
...
SELECT CAST('MT-' + [mt].[MEMBER_TYPE] + ':' + [n].[ID] AS VARCHAR(50)) AS [GroupMemberId],
       ...
       'Public Groups' AS [ParentEntityId],
       'MT' AS [GroupType],
       [mt].[DESCRIPTION] AS [GroupName],
       [mt].[DESCRIPTION] AS [GroupDescription],
       CAST(1 AS BIT) AS [IsSimpleGroup],
       [mt].[MEMBER_RECORD] AS [IsMemberGroup],
       [n].[ID] AS [PartyId],
       [n].[ID] AS [Id],
       [n].[STATUS] AS [Status],
       ...
       CASE WHEN [n].[STATUS] LIKE 'D%' THEN CAST(1 AS BIT)
            ELSE CAST(0 AS BIT)
       END AS [IsMarkedForDelete],
       CAST(1 AS BIT) [IsPrimary],
       CASE WHEN [n].[MEMBER_STATUS] != 'D' THEN CAST(1 AS BIT)
            ELSE CAST(0 AS BIT)
       END AS [IsActive],
       [n].[JOIN_DATE] AS [JoinDate],
...
GO
CREATE VIEW [vSoaGroupSummary]
AS  
SELECT [Product].[PRODUCT_CODE] AS [GroupId],
       'Public' AS [ParentEntityTypeName],
       'Public Groups' AS [ParentEntityId],
       [Product].[PROD_TYPE] AS [GroupType],
       [Product].[TITLE] AS [Name],
       CAST([Product].[DESCRIPTION] AS VARCHAR(6000)) AS [Description],
       CAST(0 AS BIT) AS [IsSimpleGroup],
       CAST(0 AS BIT) AS [IsMemberGroup]
  FROM [dbo].[Product]
 WHERE ( [Product].[PROD_TYPE] IN ( 'DUES', 'SUBS', 'SEC', 'CHAPT', 'VOL', 'MISC', 'COMMITTEE' ) )
UNION ALL
...
AS 
SELECT [REFERENCE_NUM] AS [InvoiceId], 
       [INVOICE_DATE] AS [InvoiceDate], 
       [INVOICE_NUM] AS [InvoiceNumber], 
       [BT_ID] AS [BillToPartyId], 
       [BT_ID] AS [SoldToPartyId], 
       [DESCRIPTION] AS [Description], 
       [CHARGES] AS [InvoiceAmount], 
       [BALANCE] AS [Balance]
  FROM [dbo].[Invoice]
...
SELECT [p].[PRODUCT_CODE] AS [ItemId],
  ...
  FROM [dbo].[Product] AS [p]
    WHERE ([p].[PROD_TYPE] IN ('SALES', 'GIFT', 'DUES', 'SUB', 'SEC'))
UNION 
...
GO
CREATE VIEW [vSoaPartyItemPrice]
AS
SELECT     
	p.PartyId, 
	p.Id AS Id, 
...
IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[vSoaPartyRelationshipType]'))
    DROP VIEW [dbo].[vSoaPartyRelationshipType]
GO
CREATE VIEW [dbo].[vSoaPartyRelationshipType]
AS  
SELECT [Relationship_Types].[RELATION_TYPE] AS [PartyRelationshipTypeId],
       [Relationship_Types].[DESCRIPTION] AS [Description]
  FROM [dbo].[Relationship_Types]
...
IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[vSoaPartyRelationship]'))
    DROP VIEW [dbo].[vSoaPartyRelationship]
GO
CREATE VIEW [dbo].[vSoaPartyRelationship]
AS  
SELECT r.[SEQN] AS [PartyRelationshipId],
       r.[ID] AS [PartyId],
       [rt].[PartyRelationshipTypeId],
       [rt].[Description] AS [PartyRelationshipTypeDescription],
       ps.[PartyId] AS [TargetPartyId],
       ps.[Id] AS [TargetId],
       ps.[Status] AS [TargetStatus],
       ps.[Name] AS [TargetName],
       ps.[Sort] AS [TargetSort],
       ps.[CityName] AS [TargetCityName],
       ps.[CountryName] AS [TargetCountryName],
       ps.[CountrySubEntityName] AS [TargetCountrySubEntityName],
       ps.[Phone] AS [TargetPhone],
       ps.[Email] AS [TargetEmail],
       ps.[IsMarkedForDelete] AS [TargetIsMarkedForDelete],
       ' ' AS [Description],
       r.[NOTE] AS [Note],
       r.[EFFECTIVE_DATE] AS [EffectiveDate],
       r.[THRU_DATE] AS [EndDate]
  FROM [dbo].[Relationship] r
       INNER JOIN [dbo].[vSoaPartyRelationshipType] [rt] ON r.[RELATION_TYPE] = [rt].[PartyRelationshipTypeId]
       INNER JOIN [dbo].[vSoaPartySummary] ps ON r.[TARGET_ID] = ps.[PartyId]
...
IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[vSoaSystemSetting]'))
    DROP VIEW [dbo].[vSoaSystemSetting]
GO
CREATE VIEW [vSoaSystemSetting]
AS
SELECT 'CurrentServerTime' AS [SystemSettingId], 'CurrentServerTime' AS [Name], CONVERT(VARCHAR(6000),  GETDATE(), 126) AS [Value]
UNION ALL
SELECT 'ServerProductVersion' AS [SystemSettingId], 'ServerProductVersion' AS [Name], CONVERT(VARCHAR(6000), SERVERPROPERTY('ProductVersion')) AS [Value]
