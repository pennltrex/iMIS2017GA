...
@ECHO.
@ECHO ===================================================
@ECHO = iMIS 20.2.25.x Database Upgrade Driver Script    =
@ECHO ===================================================
@ECHO.
...
REM ---------------------------------------------------------- 
SET PREVVERSTR=20.2.25
SET PREVVERVALUE=200225

REM * SET IDENTIFIER VVVV------- **
SET UPGRADESCRIPTDIR=0006_DBUpgrade
...
@ECHO   User: %USERNAME%
@ECHO   AppRoot bin folder: %APPBINDIR%
@ECHO   Scheduler bin folder: %SCHEDULERBINDIR%
...
REM ---------------------------------------------------------------------------------------------
REM - Create all the new iMIS 20 database tables.  If the table already exists in this database -
...
osql -t -S %SERVERNAME% -U %USERNAME% -P %PASSWORD% -d %DBNAME% -n -e -b -r -u -w 256 -i .\ScriptFiles\%UPGRADESCRIPTDIR%\UpdateTableSchema.sql 1> .\LogFiles\%UPGRADESCRIPTDIR%\UpdateTableSchema.log 2> .\LogFiles\%UPGRADESCRIPTDIR%\UpdateTableSchema.err
IF %ERRORLEVEL% NEQ 0 (
    @echo.
...
REM ---------------------------------------------------
REM - Add all the necessary iMIS 20 database triggers -
REM ---------------------------------------------------
@ECHO.
IF "%LOGTIME%" EQU "1" ECHO %date%-%time%
...
REM -----------------------------------------------------------------
REM - After Loading Data Update UserKey References To Ensure Validity
REM -----------------------------------------------------------------
@ECHO.
IF "%LOGTIME%" EQU "1" ECHO %date%-%time%
...
REM ----------------------------------------------
REM - Clean Storage Documents that are being added
REM ----------------------------------------------
@ECHO.
IF "%LOGTIME%" EQU "1" ECHO %date%-%time%
@ECHO Cleaning Added Storage Documents ...
osql -t -S %SERVERNAME% -U %USERNAME% -P %PASSWORD% -d %DBNAME% -n -e -b -r -w 256 -i .\ScriptFiles\%UPGRADESCRIPTDIR%\CleanNewStorageDocuments.sql 1> .\LogFiles\%UPGRADESCRIPTDIR%\CleanNewStorageDocuments.log 2> .\LogFiles\%UPGRADESCRIPTDIR%\CleanNewStorageDocuments.err
IF %ERRORLEVEL% NEQ 0 (
    @echo.
    @echo *** ERROR cleaning added documents ***
    @echo.
    type ".\LogFiles\%UPGRADESCRIPTDIR%\CleanNewStorageDocuments.err"
    @echo.
    @echo *** UPGRADE PROCESS ENDED WITH ERRORS !!! ***
    @echo.
    EXIT /B 1
...
REM ---------------------------------------------------
REM - Load DocumentStorage with Added Storage Documents
REM ---------------------------------------------------
@ECHO.
IF "%LOGTIME%" EQU "1" ECHO %date%-%time%
@ECHO Adding New Storage Documents ...
SET /A ERRORFLAG=0
copy nul .\LogFiles\%UPGRADESCRIPTDIR%\NewStorageDocuments.log /y > nul
copy nul .\LogFiles\%UPGRADESCRIPTDIR%\NewStorageDocuments.err /y > nul
for %%i in (.\AsiTableData\%UPGRADESCRIPTDIR%\NewStorageDocuments*.dat) do (
    ECHO Loading %%i >> ".\LogFiles\%UPGRADESCRIPTDIR%\UpdatedDocuments.log"
    bcp %DBNAME%.dbo.DocumentStorage in "%%i" -f ".\FormatFiles\%UPGRADESCRIPTDIR%\DocumentStorage.fmt" -V 90 -S %SERVERNAME% -U %USERNAME% -P %PASSWORD% 1>> ".\LogFiles\%UPGRADESCRIPTDIR%\NewStorageDocuments.log" -E 2>> ".\LogFiles\%UPGRADESCRIPTDIR%\NewStorageDocuments.err" 
    SET /A ERRORFLAG=!ERRORFLAG! + !ERRORLEVEL!
)
IF !ERRORFLAG! NEQ 0 (
    @echo.
    @echo *** ERROR adding new storage documents ***
    @echo.
    type ".\LogFiles\%UPGRADESCRIPTDIR%\NewStorageDocuments.log"
    type ".\LogFiles\%UPGRADESCRIPTDIR%\NewStorageDocuments.err"
    @echo.
    @echo *** UPGRADE PROCESS ENDED WITH ERRORS !!! ***
    @echo.
    EXIT /B 1
...
REM ------------------------------------------------
REM - Clean Storage Documents that are being updated
REM ------------------------------------------------
@ECHO.
IF "%LOGTIME%" EQU "1" ECHO %date%-%time%
@ECHO Cleaning Updated Storage Documents ...
osql -t -S %SERVERNAME% -U %USERNAME% -P %PASSWORD% -d %DBNAME% -n -e -b -r -w 256 -i .\ScriptFiles\%UPGRADESCRIPTDIR%\CleanUpdatedStorageDocuments.sql 1> .\LogFiles\%UPGRADESCRIPTDIR%\CleanUpdatedStorageDocuments.log 2> .\LogFiles\%UPGRADESCRIPTDIR%\CleanUpdatedStorageDocuments.err
IF %ERRORLEVEL% NEQ 0 (
    @echo.
    @echo *** ERROR cleaning updated documents ***
    @echo.
    type ".\LogFiles\%UPGRADESCRIPTDIR%\CleanUpdatedStorageDocuments.err"
    @echo.
    @echo *** UPGRADE PROCESS ENDED WITH ERRORS !!! ***
    @echo.
    EXIT /B 1
...
REM -----------------------------------------------------
REM - Load DocumentStorage with updated Storage Documents
REM -----------------------------------------------------
@ECHO.
IF "%LOGTIME%" EQU "1" ECHO %date%-%time%
@ECHO Updating Existing Storage Documents ...
SET /A ERRORFLAG=0
copy nul .\LogFiles\%UPGRADESCRIPTDIR%\UpdatedStorageDocuments.log /y > nul
copy nul .\LogFiles\%UPGRADESCRIPTDIR%\UpdatedStorageDocuments.err /y > nul
for %%i in (.\AsiTableData\%UPGRADESCRIPTDIR%\UpdatedStorageDocuments*.dat) do (
    ECHO Loading %%i >> ".\LogFiles\%UPGRADESCRIPTDIR%\UpdatedDocuments.log"
    bcp %DBNAME%.dbo.DocumentStorage in "%%i" -f ".\FormatFiles\%UPGRADESCRIPTDIR%\DocumentStorage.fmt" -V 90 -S %SERVERNAME% -U %USERNAME% -P %PASSWORD% 1>> ".\LogFiles\%UPGRADESCRIPTDIR%\UpdatedStorageDocuments.log" -E 2>> ".\LogFiles\%UPGRADESCRIPTDIR%\UpdatedStorageDocuments.err" 
    SET /A ERRORFLAG=!ERRORFLAG! + !ERRORLEVEL!
)
IF !ERRORFLAG! NEQ 0 (
    @echo.
    @echo *** ERROR updating existing storage documents ***
    @echo.
    type ".\LogFiles\%UPGRADESCRIPTDIR%\UpdatedStorageDocuments.log"
    type ".\LogFiles\%UPGRADESCRIPTDIR%\UpdatedStorageDocuments.err"
    @echo.
    @echo *** UPGRADE PROCESS ENDED WITH ERRORS !!! ***
    @echo.
    EXIT /B 1
...
REM --------------------------------------------------
REM - Update Task Definition and Notification Set Data 
REM --------------------------------------------------
@ECHO.
IF "%LOGTIME%" EQU "1" ECHO %date%-%time%
@ECHO Updating Task Definition Data ...
osql -t -S %SERVERNAME% -U %USERNAME% -P %PASSWORD% -d %DBNAME% -n -e -b -r -h-1 -w 256 -i .\ScriptFiles\%UPGRADESCRIPTDIR%\UpdateTaskDefinitionData.sql 1> .\LogFiles\%UPGRADESCRIPTDIR%\UpdateTaskDefinitionData.log 2> .\LogFiles\%UPGRADESCRIPTDIR%\UpdateTaskDefinitionData.err
IF %ERRORLEVEL% NEQ 0 (
    @echo.
    @echo *** ERROR updating task definition data ***
    @echo.
    type ".\LogFiles\%UPGRADESCRIPTDIR%\UpdateTaskDefinitionData.err"
    @echo.
    @echo *** UPGRADE PROCESS ENDED WITH ERRORS !!! ***
    @echo.
    EXIT /B 1
...
REM ===========================================================================
REM = End of Document Update Code =============================================
REM ===========================================================================
...
SET /A ERRORFLAG=%ERRORFLAG% + %ERRORLEVEL%
osql -t -S %SERVERNAME% -U %USERNAME% -P %PASSWORD% -d %DBNAME% -n -e -b -r -Q "DELETE FROM [dbo].[LicenseRef]" 1>> .\LogFiles\%UPGRADESCRIPTDIR%\PrepareLicenseUpdate.log 2>> .\LogFiles\%UPGRADESCRIPTDIR%\PrepareLicenseUpdate.err
SET /A ERRORFLAG=%ERRORFLAG% + %ERRORLEVEL%
IF %ERRORFLAG% NEQ 0 (
    @echo.
...
for %%i in (LicenseRef,LicenseLegacyMap,UniformLicense) do (
...
REM -----------------------------------------------------------------------
REM - Generate a list of all the dynamic BOs so they can be republished
REM -----------------------------------------------------------------------
@ECHO.
IF "%LOGTIME%" EQU "1" ECHO %date%-%time%
@ECHO Generating A List Of All Dynamic Business Objects ...
IF EXIST .\ScriptFiles\%UPGRADESCRIPTDIR%\ListOfDynamicBOs.txt ATTRIB -r .\ScriptFiles\%UPGRADESCRIPTDIR%\ListOfDynamicBOs.txt
osql -t -S %SERVERNAME% -U %USERNAME% -P %PASSWORD% -d %DBNAME% -n -h-1 -b -r -w 8196 -i .\ScriptFiles\%UPGRADESCRIPTDIR%\GenerateListOfDynamicBOs.sql 1> .\ScriptFiles\%UPGRADESCRIPTDIR%\ListOfDynamicBOs.txt 2> .\LogFiles\%UPGRADESCRIPTDIR%\GenerateListOfDynamicBOs.err
IF %ERRORLEVEL% NEQ 0 (
    @echo.
    @echo *** ERROR generating a list of all dynamic business objects ***
    @echo.
    type ".\LogFiles\%UPGRADESCRIPTDIR%\GenerateListOfDynamicBOs.err"
    @echo.
    @echo *** UPGRADE PROCESS ENDED WITH ERRORS !!! ***
    @echo.
    EXIT /B 1
...
REM --------------------------------------------------------------------
REM - Build and Publish all Dynamic BOs so they pick up all code changes
REM --------------------------------------------------------------------
@ECHO.
IF "%LOGTIME%" EQU "1" ECHO %date%-%time%
@ECHO Building And Publishing Dynamic Business Objects ...
SET /A BOPUBLISHERRORCOUNT = 0
copy nul .\LogFiles\%UPGRADESCRIPTDIR%\BuildPublishDynamicBOs.log /y > nul
copy nul .\LogFiles\%UPGRADESCRIPTDIR%\BuildPublishDynamicBOs.err /y > nul
for /F "tokens=1 delims= " %%f in (.\ScriptFiles\%UPGRADESCRIPTDIR%\ListOfDynamicBOs.txt) do SET LISTOFOBJECTS=%%f
if "%LISTOFOBJECTS%" NEQ "" (
    @echo Processing... 
    for %%i in ("%LISTOFOBJECTS:|=" "%") do (
        "%APPBINDIR%\BuildPublishBOs.exe" -CS %SERVERNAME% -CU %USERNAME% -CP %PASSWORD% -CD %DBNAME% -O %%~i  | .\Utilities\Tee /A /FOE:: .\LogFiles\%UPGRADESCRIPTDIR%\BuildPublishDynamicBOs.log 
    )
) else (
...
IF %ERRORLEVEL% NEQ 0 (
    @echo.
    @echo     Error building and publishing dynamic business objects... 
...
"%APPBINDIR%\BuildPublishBOs.exe" -W -S -CS %SERVERNAME% -CU %USERNAME% -CP %PASSWORD% -CD %DBNAME%  | .\Utilities\Tee /FOE:: .\LogFiles\%UPGRADESCRIPTDIR%\BuildPublishBOs.log
IF %ERRORLEVEL% NEQ 0 (
    @echo.
...
REM --------------------------------------------------------------------------
REM - Clean out ASI Translation Dictionaries
REM --------------------------------------------------------------------------
@ECHO.
IF "%LOGTIME%" EQU "1" ECHO %date%-%time%
@ECHO Cleaning Translation Dictionaries ...
osql -t -S %SERVERNAME% -U %USERNAME% -P %PASSWORD% -d %DBNAME% -n -e -b -r -w 256 -i .\ScriptFiles\%UPGRADESCRIPTDIR%\CleanAsiTranslationDictionaries.sql 1> .\LogFiles\%UPGRADESCRIPTDIR%\CleanAsiTranslationDictionaries.log 2> .\LogFiles\%UPGRADESCRIPTDIR%\CleanAsiTranslationDictionaries.err
IF %ERRORLEVEL% NEQ 0 (
    @echo.
    @echo *** ERROR cleaning translation dictionaries ***
    @echo.
    type ".\LogFiles\%UPGRADESCRIPTDIR%\CleanAsiTranslationDictionaries.err"
    @echo.
    @echo *** UPGRADE PROCESS ENDED WITH ERRORS !!! ***
    @echo.
    EXIT /B 1
...
REM --------------------------------------------------------------------------
REM - Bulk Load ASI Translation Dictionaries
REM --------------------------------------------------------------------------
@ECHO.
IF "%LOGTIME%" EQU "1" ECHO %date%-%time%
@ECHO Loading Updated Translation Dictionaries ...
osql -t -S %SERVERNAME% -U %USERNAME% -P %PASSWORD% -d %DBNAME% -n -e -b -r -u -w 256 -i .\ScriptFiles\%UPGRADESCRIPTDIR%\MergeLocalizationVersionData.sql 1> .\LogFiles\%UPGRADESCRIPTDIR%\LocalizationVersion.log 2> .\LogFiles\%UPGRADESCRIPTDIR%\LocalizationVersion.err
IF %ERRORLEVEL% NEQ 0 (
    @echo.
    @echo *** loading updated translation dictionaries ***
    @echo.
    type ".\LogFiles\%UPGRADESCRIPTDIR%\LocalizationVersion.log"
    type ".\LogFiles\%UPGRADESCRIPTDIR%\LocalizationVersion.err"
    @echo.
    @echo *** UPGRADE PROCESS ENDED WITH ERRORS !!! ***
    @echo.
    EXIT /B 1
)
bcp %DBNAME%.dbo.LocalizationVersionPhrase in ".\AsiTableData\%UPGRADESCRIPTDIR%\LocalizationVersionPhrase.dat" -f ".\FormatFiles\%UPGRADESCRIPTDIR%\LocalizationVersionPhrase.fmt" -V 90 -S %SERVERNAME% -U %USERNAME% -P %PASSWORD% 1>> ".\LogFiles\%UPGRADESCRIPTDIR%\LocalizationVersionPhrase.log" -E 2>> ".\LogFiles\%UPGRADESCRIPTDIR%\LocalizationVersionPhrase.err" 
IF %ERRORLEVEL% NEQ 0 (
    @echo.
    @echo *** loading updated translation dictionaries ***
    @echo.
    type ".\LogFiles\%UPGRADESCRIPTDIR%\LocalizationVersionPhrase.log"
    type ".\LogFiles\%UPGRADESCRIPTDIR%\LocalizationVersionPhrase.err"
    @echo.
    @echo *** UPGRADE PROCESS ENDED WITH ERRORS !!! ***
    @echo.
    EXIT /B 1
...
REM --------------------------------------------------------------------------
REM - Update Translation Phrases
REM --------------------------------------------------------------------------
@ECHO.
IF "%LOGTIME%" EQU "1" ECHO %date%-%time%
@ECHO Updating Translation Phrases ...
osql -t -S %SERVERNAME% -U %USERNAME% -P %PASSWORD% -d %DBNAME% -n -e -b -r -w 256 -i .\ScriptFiles\%UPGRADESCRIPTDIR%\UpdateTranslationPhrases.sql 1> .\LogFiles\%UPGRADESCRIPTDIR%\UpdateTranslationPhrases.log 2> .\LogFiles\%UPGRADESCRIPTDIR%\UpdateTranslationPhrases.err
IF %ERRORLEVEL% NEQ 0 (
    @echo.
    @echo *** ERROR updating translation phrases ***
    @echo.
    type ".\LogFiles\%UPGRADESCRIPTDIR%\UpdateTranslationPhrases.err"
    @echo.
    @echo *** UPGRADE PROCESS ENDED WITH ERRORS !!! ***
    @echo.
    EXIT /B 1
...
REM -----------------------------------------------------------------
REM - Ensure the necessary SQL logins for iMIS desktop client exist -
REM -----------------------------------------------------------------
@ECHO.
IF "%LOGTIME%" EQU "1" ECHO %date%-%time%
@ECHO Ensuring SQL Logins Are Correct ...
...
IF %ERRORLEVEL% NEQ 0 (
    @echo.
    @echo *** ERROR ensuring SQL logins are correct ***
    @echo.
...
REM -----------------------------
REM - Resetting Encryption Keys -
REM -----------------------------
@ECHO.
IF "%LOGTIME%" EQU "1" ECHO %date%-%time%
@ECHO Resetting Encryption Keys ...
IF NOT EXIST "%SCHEDULERBINDIR%\ResetEncryption.exe" COPY "%APPBINDIR%\ResetEncryption.exe" "%SCHEDULERBINDIR%\ResetEncryption.exe"
IF NOT EXIST "%SCHEDULERBINDIR%\ResetEncryption.exe.config" COPY "%APPBINDIR%\ResetEncryption.exe.config" "%SCHEDULERBINDIR%\ResetEncryption.exe.config"
"%SCHEDULERBINDIR%\ResetEncryption.exe" -CS %SERVERNAME% -CU %USERNAME% -CP %PASSWORD% -CD %DBNAME% 1> .\LogFiles\%UPGRADESCRIPTDIR%\ResetEncryption.log 2> .\LogFiles\%UPGRADESCRIPTDIR%\ResetEncryption.err
IF %ERRORLEVEL% NEQ 0 (
    @echo.
    @echo *** ERROR resetting encryption keys ***
    @echo.
    type ".\LogFiles\%UPGRADESCRIPTDIR%\ResetEncryption.err"
    @echo.
    @echo *** PROCESS ENDED WITH ERRORS !!! ***
    @echo.
    EXIT /B 1
...
@ECHO.
@ECHO		%0 appBinDir serverName serverName databaseName userName password schedulerBinDir
@ECHO.
@ECHO Where:
@ECHO	appBinDir = absolute path to the Bin directory of root install of iMIS
@ECHO	serverName = Name of the SQL Server instance to use
@ECHO 	databaseName = Name of the Database to use
@ECHO 	userName = SQL Server user name to use for login
@ECHO 	password = The password for the SQL Server user login
@ECHO   schedulerBinDir = absolute path to the Bin directory of the iMIS Scheduler
@ECHO.
EXIT /B 1 
